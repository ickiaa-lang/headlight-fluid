...
(function () {
  ...
  style.textContent = `
    ...
    #tc-joy-move { width: 130px; height: 130px; left: 20px; bottom: max(20px, env(safe-area-inset-bottom)); }
    #tc-joy-move .tc-joy-thumb { width: 52px; height: 52px; }
    #tc-joy-pitch { width: 100px; height: 100px; right: 200px; bottom: max(20px, env(safe-area-inset-bottom)); }
    ...
    #tc-fire {
      ...
      right: 200px; bottom: max(140px, calc(env(safe-area-inset-bottom) + 140px));
  ...
  function syncVisibility() {
    // `gameActive` is a top-level `let` in game.js — classic <script> tags
    // share one global lexical environment, so it's readable here by name
    // (it is NOT a property of `window`, so window.gameActive would be undefined).
    const active = (typeof gameActive !== 'undefined') && !!gameActive;
    if (active !== wasActive) {
